...
def grow_tree(data,category,parent,attrs_names,threshold):
        ...
        if max(division)==0:
            ...
            if isnum(data[index_selected]):
                ...
                for i in range(len(category)):
                    if not data[index_selected][i]=="?":
                        ...
                        if threshold is None:
                            threshold = float(div_point)
                        if float(data[index_selected][i])<threshold:
                ...
                if len(l_son_category)>0 and len(r_son_category)>0:
                    ...
                    grow_tree(l_son_data,l_son_category,son,attrs_names,threshold)
                    son=ET.SubElement(parent,name_selected,{'value':str(div_point),"flag":"r","p":str(round(1-p_l,3))})
                    grow_tree(r_son_data,r_son_category,son,attrs_names,threshold)
                else:
                    num_max=0
                    for cat in set(category):
                        num_cat=category.count(cat)
                ...
                for k in set(data[index_selected]):
                    if not k=="?":
                        ...
                        grow_tree(son_data,son_category,son,attrs_names,threshold)   
    else:
